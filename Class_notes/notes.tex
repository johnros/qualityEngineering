...
The first, and most basic PCR is the $C_p$.

\begin{definition}[$C_p$]
\begin{align}
	C_p:= \frac{USL-LSL}{6 \sigma}, 
\end{align}
where $\sigma$ is the standard deviation of the CTQ.

Assuming $LSL$ and $USL$ are symmetric about the target value, we have $\Delta:= \frac{1}{2}(USL-LSL)$, and thus
\begin{align}
	C_p:= \frac{2 \Delta}{6 \sigma}, 
\end{align}
\end{definition}
Clearly $C_p$ is a process parameter, that needs to be estimated.
\begin{definition}[$\hat{C}_p$]
\begin{align}
...
[TODO]
Obviously, there is a relation between $C_p$ and the probability for non-compliance, we shall denote as $p_{NC}$.
\begin{align}
	p_{NC} &:= P(CTQ < LSL \union CTQ > USL)	\\	
	&:= P(CTQ < LSL) + P(CTQ > USL) 
\end{align}
Assuming $CTQ$ is symmetrically distributed:
\begin{align}
	P(CTQ < LSL) + P(CTQ > USL) &= 2 P(CTQ < LSL)
\end{align}
Assuming $CTQ$ is Gaussian distributed about its target value $\mu$, and denoting $Z \sim \gauss{0,1}$:
\begin{align}
	2 P(CTQ < LSL) &= 2 P(Z < \frac{-\Delta}{\sigma}) \nonumber \\ 
	&= 2 P(Z < \frac{-C_p}{3}) \label{eq:cp_and_non_conformance}
\end{align}
Now verifying Eq.(\ref{eq:cp_and_non_conformance}), by assuming a 3-sigma process that exhausts its specification, we get: 
$\Delta=3 \sigma \Rightarrow C_p=1$
