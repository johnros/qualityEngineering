...
\begin{example}[Identifiability]
The problem demonstrated in Example~\ref{eg:likelihood_of_failures} is known in the statistical literature as an \emph{identifiability} problem. 
\end{example}
...
Since we are in the business of improving quality via improving reliability, we want to know what affects failure rates. 
We will thus introduce now covariates, or factors (in the DOE parlance), in the hope of estimating their effects.


\subsubsection{Accelerated Life Models}
...
An accelerate life mode assumes that the effect of covariates are multiplicative. For this reason, a change in a covariate ``accelerates life'' by the factor of its effect. 
Let's be clear.
Such models imply that $\log \T = x'\beta + \varepsilon \Leftrightarrow \T = $


\begin{extra}[Accelrated life and generalized linear models]
Fans of generalized linear models (GLM) may recognize the accelerated life model as a exponential regression with canonical link function.
\end{extra}
