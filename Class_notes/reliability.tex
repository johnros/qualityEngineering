...
We call a failure time distribution to be in the \emph{increasing failure rate} (IFR) class, if it has a non decreasing failure rate.
\end{definition}
...
We call a failure time distribution to be in the \emph{increasing failure rate average} (IFRA) class, if 
$\cuhazard{\T}{t}/t$ is non decreasing in $t$.
\end{definition}

\begin{definition}[NBU]
We call a failure time distribution to be in the \emph{new better then used} (NBU) class, if 
...
We call a failure time distribution to be in the \emph{new better then used in expectation} (NBUE) class, if 
$\mu(t) \leq \mu(0)$.
\end{definition}
