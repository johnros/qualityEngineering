...
For instance, the lab may produce conditions where time (assumingly) moves ten times as fast than in real-life operating conditions.

Formally if $\T_1$ is the time to failure under conditions $x_1$ , and $\T_2$ under conditions $x_2$. 
And the covariates effects are such that $x_1' \beta=1$ and $x_2' \beta=\gamma$,  then 
\begin{align}
...
Qualitatively, both either accelerate or decelerate time. Quantitatively, the exact amount of acceleration/deceleration may differ. 
...
The exponential distribution assumption is thus closed under the various time rescaling.
\end{example}
...
We now relax the explicit model assumption, and adopt a more flexible semi-parametric model class, known as the \emph{piecewise exponential class}.\marginnote{Piecewise Exponenitial}
