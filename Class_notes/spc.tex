...
\subsection{Other Multivariate Statistics}
...
\item The signal we seek is not in the mean of the CTQ but rather in its variance, such as Example~\ref{eg:irrigation}. Put differently, we actually want a multivariate version of the $s$ chart and not the \barxChart. 

\item The signal we seek is expected to exist in a small subset of coordinates, like in the cyber-security example (\ref{eg:cyber}). This is known as a \emph{sparse signal}, unlike the \emph{dense signal} targeted by the \tsq. \marginnote{Sparse Signal}

\item \tsq is incomputable. From Eq.\ref{eq:hotteling} we see that computing \tsq required the inversion of the covariance matrix. Often this is an impossible task. We call this a \emph{high-dimensional problem}.
\end{enumerate}
