...
It is also been re-popularized in the context of \emph{condition based maintenance}, where maintenance is not performed periodically, but rather, when certain conditions are met. 

A process is said to be in \emph{statistical control}, \aka \emph{normal operating conditions} (NOC), if all its variation is attributable to chance causes, i.e., \emph{noise}.
If this is not the case, we call it \emph{out of control} and we will seek the \emph{assignable causes}, so that we may reduce variability by removing them.
All the statistical tools of chapters \ref{sec:exploratory} and \ref{sec:inference} may be called upon for this endeavor but in this chapter we focus on one particular such tool- the \emph{control chart}.
...
\begin{think}
	In SPC we call it ``in-control'' and ``out-of-control''.
	How do we call these in the hypothesis testing parlance?
\end{think}
...
\end{align} 
Assuming $x_t \sim \gauss{\mu, \sigmabar^2}$ then if under control then $C_t \sim \gauss{0, t \sigmabar^2}$, we could thus set 
\begin{align}
\label{eq:cusum_simple_limits}
\begin{split}
	UCL &:=  \arm \sigma_{C_t}=  \arm \, \sqrt{t \sigmabar^2},  \\
	LCL &:=  - \arm \sigma_{C_t}=  - \arm \, \sqrt{t \sigmabar^2},
\end{split}
\end{align}
and $\arm=3$ as usual. 
...
The \emph{Frobenius norm} of matrix $A$, \aka the \emph{Hilbert-Schmidt norm}, or \emph{Schur norm} is defined as:\marginnote{Hilbert-Schmidt norm,\\ Schur norm}
\begin{align}
...
Denoting the Euclidean distance of a vector $x$ from zero by $\norm{x}_{Euc}$, we can denote a matrix's \emph{spectral norm}, \aka \emph{operator norm}, or \emph{induced norm}, by:\marginnote{Operator norm, \\ Induced norm}
\begin{align}
