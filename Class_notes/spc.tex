...
It is possible, that the deterioration is mild, so that it is not picked up by any CTQ individually (low power), but could have been noticed were we to aggregate signal from all CTQs. This is the concern of the current section. 

\subsection{The Mass Univariate Approach}
...
\begin{align}
	T^2_t := n (\bar{x}_t-\mu_0)' \hat{\Sigma}_t^{-1} (\bar{x}_t-\mu_0).
\end{align}
If $\x_{it}$ is $p$-variate Gaussian distributed, $\gauss{\mu_0, \Sigma_{p \times p}}$, then 
\begin{align}
	T^2_t \sim \chi^2_p.
\end{align}
We can thus construct the control limit:
\begin{align}
	UCL:= \chi^2_{1-\alpha/2,p}.
\end{align}
Note that the \tsq statistic is non directional: it will increase in the presence of positive or negative drift. 
