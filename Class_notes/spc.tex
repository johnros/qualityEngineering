...
Operator $p$-norms view the matrix as a the linear function it represents, so that the distance of a matrix from the origin is captured by the displacement this linear function causes upon its (normalized) inputs.
...
\begin{align}
	\norm{a}_p:= (\sum |a_j|^p)^{1/p}
\end{align}
The general form of the matrix operator $p$-norm is thus
\begin{align}
	\norm{A}_p:= \sup_x \set{\norm{Ax}_p; \norm{x}_p=1}.
\end{align}
...
\end{align}
As such, it can be seen as a \emph{vector} $2$-norm, that stacks the matrix into a single vector, thus ignoring its ``matrixy'' structure. 
\end{definition}
