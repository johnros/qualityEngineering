...
\begin{align}
	t^2(x_t)=(\bar{x}_t-\mu_0) (s^2(x))^{-1} (\bar{x}_t-\mu_0).
\end{align}
This notation readily extends to the multivariate case. 
For $p$ CTQs, then $\bar{x}_t$ and $\mu_0$ are $p$-length vectors, and $s^2(x)$ is replaced with the $p \times p$ covariance matrix $\hat{\Sigma}(x)$.
...
We opted the $\chi^2_p$ approximation for ease of exposition. 
For exact result \cite[Ch.7]{qiu_introduction_2013} should be consulted. 
\end{remark}


\subsection{Drill Down}
Your control chart just went outside the control limits and an alarm has rung.
...
There are, however, circumstances where we should choose a different statistic.
...
As we will see, the way to deal with many of the problem that arise with multivariate data, is to construct $p$ \textbf{univariate} tests, and them combine them; even at the cost of ignoring the correlations between coordinates, even if that was the original motivation for the \tsq. 
\begin{description}
...
\item [High dimension] \tsq is simply \textbf{incomputable}. From Eq.(\ref{eq:hotteling}) we see that computing \tsq required the inversion of the covariance matrix. In the case that $p>\tau$ it will be impossible to invert $\Sigma$.

\item [Sparse signal] The signal we seek is expected to exist in a small subset of the $p$ coordinates, like in the cyber-security of Example~\ref{eg:cyber}. This is known as a \emph{sparse signal}, unlike the \emph{dense signal} targeted by the \tsq. \marginnote{Sparse Signal}
...
To do so, we recall that in vector spaces, \emph{norm functions} measure distance of a vector  to the origin (the $0$ vector).
...
To define the \emph{matrix} $p$-norm, we need to recall the $p$-norm of a \emph{vector} $a$, which generalizes the idea of Euclidean distances:
\begin{align}
	\norm{a}_p:= \left( \sum_j |a_j|^p \right)^{1/p}
\end{align}
You may verify that the standard Euclidean norm is given by the vector $2$-norm.
Using vector $p$-norms, then a matrix $p$-norm is defined as 
\begin{align}
	\norm{A}_p:= \sup_x \set{\norm{Ax}_p; \norm{x}_p=1}.
\end{align}
\end{definition}
...
\end{align}
But wait! 
$D^{-1}_{jj}=(\Sigma_{jj})^{-1}$ is just the inverse variance of the $j$'th coordinate: $\frac{1}{Var(x_{1,j},\dots,x_{n,j})}$.
...
Denoting the z-statistic of the $j$'th coordinate by $z_j:= \frac{(\bar{x}_j - \mu_{0,j})^2}{\var{\bar{x}_j}}$ we can now easily design many high dimensional signal detectors in the form of
\begin{align}
\label{eq:marginal_test}
	 	T(x) :=  f(z_1,\dots,z_p) .
\end{align}
where $f$ is some function measuring distance from the $\mu_0$ vector.
Clearly, for $T^{SD}$ we need to set $f(z_1,\dots,z_p) = \normII{(z_1,\dots,z_p)}^2$.
...
Denoting by IC and OC the system In Control and Out of Control, respectively, then the parameters governing the solution are:
\begin{description}
