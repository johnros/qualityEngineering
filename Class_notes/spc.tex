...
\item [High dimension] \tsq is simply \textbf{incomputable}. From Eq.(\ref{eq:hotteling}) we see that computing \tsq required the inversion of the covariance matrix. In the case that the number of CTQs is greater than the sample size with which the covariance was estimated ($p>\tau$) it will be impossible to invert $\Sigma$.
