...
\subsection{Other estimators of the $\cp$}
The first $\cp$ estimator we proposed used $s$ to estimate the $\sigma$ (Eq.\ref{eq:cpHat}).
While a seemingly very natural estimator, historically, $\sigma$ was more often estimated using the (scaled) range of the data:
 $\range{x}:= \max\set{x}-\min\set{x}$ (Definition~\ref{def:range}).
The range is indeed a scale summary, and may be easier to compute than $s$ is a computer is not around. 

The $\cp$ estimator in Eq.(\ref{eq:cpHat}) is actually known as the $\hat{P}_p$ \emph{performance index}.\marginnote{Performance Index}
...
This is quite surprising as we already know that $s$ is \textbf{not} a robust location measure. 
It is true, however, that estimating $\sigma$ with $\range{x}$ is very sensitive to the existence of defects, while $s$ less so.

If robustness is indeed of interest, i.e., if we want to estimate the process capability in the presence of defects, one may consider the following truly \textbf{robust} capability estimators:
\begin{align}
...
When using a robust capacity estimator, attention should be paid to its interpretation. 
