...
In the BI world, these are typically known as \emph{key performance indicators} (KPI). \marginnote{KPI}
...
SPC that triggers an intervention in a SCADA system\footnote{Supervisory Control and Data Acquisition refers to systems than integrate sensors and computers for fully, or semi, automated Industrial Control (ICS). 
SCADA and ICS recently became famous in the context of the STUXNET worm, since STUXNET target the SCADA system controlling uranium enriching centrifuges.}. 
...
\begin{extra}[Smoking and Cancer]
	If, like me, you are curious to understand how did we miss the cancerous effects of smoking, have a look at \cite{cornfield_smoking_2009}.
\end{extra}
...
\begin{remark}[PPM of Six-Sigma]
In the six-sigma literature you will find the following table:
\begin{figure}[H]
\centering
\includegraphics[width=0.2\linewidth]{art/six_sigma_table}
\caption[Defects PPM at different productions sigmas.]{\footnotesize 
Defect PPM of varying sigma processes. \newline
Source: Souce: \url{http://www.whatissixsigma.net/10-things-about-six-sigma/} }
\label{fig:six_sigma_table}
\end{figure}
As you may notice, the 3-sigma and the 6-sigma defect PPMs in the table are different than the ones in Eq.(\ref{eq:3_sigma}).
This is explained in \cite[p.29]{montgomery_introduction_2007}:
\begin{quote}
...
disturbances that could cause it to shift by as much as $1.5$ standard deviations off target.
\end{quote}
This means that the convention is to compute Eq.(\ref{eq:3_sigma}) while replacing the $\expect{CTQ}=\targetValue$ assumption by $|\expect{CTQ}-\targetValue| \leq 1.5 \sigma$. 
\end{remark}
